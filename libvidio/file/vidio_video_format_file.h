...
#ifndef LIBVIDIO_VIDIO_VIDEO_FORMAT_FILE_H
#define LIBVIDIO_VIDIO_VIDEO_FORMAT_FILE_H
...
struct vidio_video_format_file : public vidio_video_format
{
public:
  vidio_video_format_file(uint32_t width, uint32_t height,
                          vidio_pixel_format pixel_format,
                          std::optional<vidio_fraction> framerate);

  vidio_video_format_file(const vidio_video_format_file& fmt);

#if WITH_JSON
  explicit vidio_video_format_file(const nlohmann::json& json);
#endif
...
#endif //LIBVIDIO_VIDIO_VIDEO_FORMAT_FILE_H
